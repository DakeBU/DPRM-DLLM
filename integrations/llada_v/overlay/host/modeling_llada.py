...
from llava.dprm_generation import apply_dprm_scores, is_dprm_remasking, write_order_trace
...
class LLaDAModelLM(LLaDAPreTrainedModel):
    ...
    def generate_with_embeds(self, inputs_embeds, steps=128, gen_length=128, block_length=128, temperature=0.,
        ...
        with torch.cuda.amp.autocast(enabled=True):
            dprm_table = kwargs.pop("dprm_table", None)
            dprm_guidance_scale = kwargs.pop("dprm_guidance_scale", None)
            dprm_ready_count = kwargs.pop("dprm_ready_count", None)
            dprm_switch_steps = kwargs.pop("dprm_switch_steps", None)
            dprm_warmup_steps = kwargs.pop("dprm_warmup_steps", None)
            dprm_force_full = bool(kwargs.pop("dprm_force_full", False))
            dprm_context_bin = int(kwargs.pop("dprm_context_bin", 0))
            trace_order_stats = kwargs.pop("trace_order_stats", None)
            trace_num_phases = int(kwargs.pop("trace_num_phases", 8))
            trace_confidence_bins = int(kwargs.pop("trace_confidence_bins", 16))
            trace_aux_bins = int(kwargs.pop("trace_aux_bins", 16))
            trace_sample_id = kwargs.pop("dprm_trace_sample_id", None)
            trace_task = kwargs.pop("dprm_trace_task", None)
            trace_doc_id = kwargs.pop("dprm_trace_doc_id", None)
            eot_token_ids = self._resolve_eot_token_ids(tokenizer, kwargs.pop("eot_token_ids", None))
            eot_suppress_until_step = int(kwargs.pop("eot_suppress_until_step", -1))
            sacm_anchor_window = int(kwargs.pop("sacm_anchor_window", 0))
            sacm_downweight = float(kwargs.pop("sacm_downweight", 1.0))
            sacm_until_step = int(kwargs.pop("sacm_until_step", -1))
            ...
            assert steps % num_blocks == 0
            steps = steps // num_blocks
            total_dprm_steps = max(num_blocks * steps, 1)
            ...
            for num_block in range(num_blocks):
                ...
                for i in range(steps):
                    ...
                    if eot_token_ids and int(num_block * steps + i) < int(eot_suppress_until_step):
                        logits = logits.clone()
                    ...
                    # Get proposal and bucket confidence scores. DPRM-random keeps a random
                    # base proposal but still buckets by the model's token confidence.
                    bucket_x0_p = None
                    if remasking in {'low_confidence', 'progressive_confidence', 'entropy', 'dprm_confidence_warmup', 'dprm_random_warmup', 'dprm_confidence', 'dprm_random', 'dprm'}:
                        p = F.softmax(logits.to(torch.float64), dim=-1) # shape (1, l + gen_length + suffix_len, vocab_size)
                        bucket_x0_p = torch.squeeze(
                        ...
                        if remasking == 'entropy':
                            x0_p = 1.0 - bucket_x0_p
                        elif remasking in {'dprm_random_warmup', 'dprm_random'}:
                            x0_p = torch.rand((x0.shape[0], x0.shape[1]), device=x0.device)
                        else:
                            x0_p = bucket_x0_p
                    elif remasking in {'random'}:
                        p = F.softmax(logits.to(torch.float64), dim=-1)
                        bucket_x0_p = torch.squeeze(
                    ...
                    confidence = torch.where(mask_index, x0_p, -np.inf)
                    bucket_confidence = torch.where(mask_index, bucket_x0_p, -np.inf)
                    if (
                        int(sacm_anchor_window) > 0
                        and suffix_len > 0
                        and float(sacm_downweight) < 1.0
                        and int(num_block * steps + i) < int(sacm_until_step)
                    ):
                        positions = torch.arange(confidence.shape[1], device=confidence.device)
                        anchor_start = int(inputs_embeds.shape[1]) + int(gen_length)
                        distance_to_anchor = anchor_start - positions
                        near_anchor = (distance_to_anchor > 0) & (distance_to_anchor <= int(sacm_anchor_window))
                        if near_anchor.any():
                            confidence[:, near_anchor] = confidence[:, near_anchor] * float(sacm_downweight)
                    if is_dprm_remasking(remasking):
                        dprm_diagnostics = {}
                        confidence = apply_dprm_scores(
                            remasking=remasking,
                            base_score=confidence,
                            bucket_confidence=bucket_confidence,
                            dprm_table=dprm_table,
                            step_index=num_block * steps + i,
                            total_steps=total_dprm_steps,
                            prompt_length=inputs_embeds.shape[1],
                            gen_length=gen_length,
                            dprm_guidance_scale=dprm_guidance_scale,
                            dprm_ready_count=dprm_ready_count,
                            dprm_switch_steps=dprm_switch_steps,
                            dprm_warmup_steps=dprm_warmup_steps,
                            dprm_force_full=dprm_force_full,
                            predicted_token_ids=x0,
                            eot_token_ids=eot_token_ids,
                            dprm_context_bin=dprm_context_bin,
                            diagnostics=dprm_diagnostics,
                        )
                    else:
                        dprm_diagnostics = None
                    ...
                    for j in range(confidence.shape[0]):
                        ...
                        transfer_index[j, select_index] = True
                    write_order_trace(
                        trace_path=trace_order_stats,
                        remasking=remasking,
                        selected_mask=transfer_index,
                        bucket_confidence=bucket_confidence,
                        step_index=num_block * steps + i,
                        total_steps=total_dprm_steps,
                        block_idx=num_block,
                        ...
                        trace_num_phases=trace_num_phases,
                        trace_confidence_bins=trace_confidence_bins,
                        trace_aux_bins=trace_aux_bins,
                        selected_token_ids=x0,
                        eot_token_ids=eot_token_ids,
                        dprm_diagnostics=dprm_diagnostics,
                    )
    ...
    @staticmethod
    def _resolve_eot_token_ids(tokenizer, raw_ids=None):
        ids = []
        if raw_ids is not None:
            if isinstance(raw_ids, int):
                ids.append(raw_ids)
            elif isinstance(raw_ids, str):
                ids.extend(int(x) for x in raw_ids.replace(",", " ").split() if x.strip())
            else:
                ids.extend(int(x) for x in raw_ids)
    ...
    def forward(
        ...
        def forward_process_embeds(input_embeds, labels, eps=1e-3):
            ...
            order_policy = os.environ.get("DPRM_TRAIN_ORDER_POLICY", "random").lower()
            def _dprm_float_env(name, default):
                try:
                    return float(os.environ.get(name, default))
                except (TypeError, ValueError):
                    return default

            conf_pos_weight = max(0.0, min(1.0, _dprm_float_env("DPRM_CONF_POSITION_WEIGHT", 0.90)))
            random_pos_weight = max(0.0, min(1.0, _dprm_float_env("DPRM_RANDOM_POSITION_WEIGHT", 0.65)))
            valid_mask = labels != -100
            if order_policy in {"random", "baseline_random", "random_ordered"}:
                order_score = torch.rand((b, l), device=input_embeds.device)
            else:
                valid_counts = valid_mask.sum(dim=1, keepdim=True).clamp_min(1)
                valid_ranks = valid_mask.to(torch.float32).cumsum(dim=1)
                rel_pos = (valid_ranks - 0.5) / valid_counts
                rel_pos = torch.where(valid_mask, rel_pos, torch.rand((b, l), device=input_embeds.device))
                # Higher order_score means the token is revealed earlier in the reverse process.
                text_order_score = 1.0 - rel_pos
                if order_policy in {"progressive", "progressive_confidence", "low_confidence"}:
                    order_score = text_order_score + torch.rand((b, l), device=input_embeds.device) * 1e-4
                elif order_policy in {"dprm_confidence_warmup", "dprm", "dprm_confidence"}:
                    order_score = conf_pos_weight * text_order_score + (1.0 - conf_pos_weight) * torch.rand((b, l), device=input_embeds.device)
                elif order_policy in {"dprm_random_warmup", "dprm_random"}:
                    order_score = random_pos_weight * text_order_score + (1.0 - random_pos_weight) * torch.rand((b, l), device=input_embeds.device)
                else:
                    raise RuntimeError(f"Unknown DPRM_TRAIN_ORDER_POLICY: {order_policy}")
            masked_indices = order_score < p_mask
            masked_indices = masked_indices & valid_mask # Combine random encoding and valid encoding
            empty_rows = valid_mask.any(dim=1) & (~masked_indices.any(dim=1))
            if empty_rows.any():
                fallback_scores = order_score.masked_fill(~valid_mask, float("inf"))
                fallback_idx = fallback_scores.argmin(dim=1)
                masked_indices[empty_rows, fallback_idx[empty_rows]] = True
