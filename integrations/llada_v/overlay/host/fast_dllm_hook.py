...
class FastDLLMGenerationHook:
                ...
                while True:
                    ...
                    if i % prefix_refresh_interval == 0:
                        x0, transfer_index = self._get_transfer_index(
                            ...
                            trace_order_stats=trace_order_stats,
                            ...
                            eot_suppress_until_step=eot_suppress_until_step,
                            sacm_anchor_window=sacm_anchor_window,
                            sacm_downweight=sacm_downweight,
                            sacm_until_step=sacm_until_step,
                        )
    ...
    def _get_transfer_index(self, logits, temperature, remasking, mask_index, x, num_transfer_tokens, 
                          ...
                          step_index=0, total_steps=1, block_idx=0, prompt_length=0, gen_length=1,
                          position_offset=0, dprm_table=None, dprm_guidance_scale=None,
                          dprm_ready_count=None, dprm_switch_steps=None, dprm_warmup_steps=None,
                          dprm_force_full=False, dprm_context_bin=0, trace_order_stats=None, trace_num_phases=8,
                          trace_confidence_bins=16, trace_aux_bins=16, trace_sample_id=None,
                          trace_task=None, trace_doc_id=None, eot_token_ids=None,
                          eot_suppress_until_step=-1, sacm_anchor_window=0,
                          sacm_downweight=1.0, sacm_until_step=-1):
        """Get transfer indices for token updates during generation."""
        if eot_token_ids and int(step_index) < int(eot_suppress_until_step):
            logits = logits.clone()
        ...
        if remasking in {'low_confidence', 'progressive_confidence', 'entropy', 'dprm_confidence_warmup', 'dprm_random_warmup', 'dprm_confidence', 'dprm_random', 'dprm'}:
            ...
            bucket_x0_p = torch.squeeze(torch.gather(p, dim=-1, index=torch.unsqueeze(x0, -1)), -1)
        ...
        if (
            ...
            and int(step_index) < int(sacm_until_step)
        ):
            positions = torch.arange(confidence.shape[1], device=confidence.device) + int(position_offset)
            anchor_start = int(prompt_length) + int(gen_length)
