...
def _dprm_answer_format_bin(question: str) -> int:
    """Prompt-only routing key: choice=0, numeric=1, open short answer=2."""
    text = str(question)
...
class Llava_OneVision_LLaDA(lmms):
    ...
    def generate_until(self, requests: List[Instance]) -> List[str]:
        ...
        for chunk in chunks:
            ...
            gen_kwargs["dprm_trace_task"] = str(task)
            gen_kwargs["dprm_trace_doc_id"] = str(batched_doc_id[0])
            gen_kwargs["dprm_trace_sample_id"] = f"{task}:{split}:{batched_doc_id[0]}"
            gen_kwargs["dprm_context_bin"] = _dprm_answer_format_bin(str(batched_contexts[0]))
            try:
                with torch.inference_mode():
