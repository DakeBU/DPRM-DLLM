...
base_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data_and_model')


def _load_gosai_oracle(ckpt_path):
    """Load a LightningModel checkpoint without triggering wandb artifact download.

    The standard LightningModel.load_from_checkpoint rebuilds the model from
    hparams, which for EnformerPretrainedModel tries to download pretrained
    weights from wandb. We patch get_artifact to return a dummy that loads the
    state dict already saved in our checkpoint instead.
    """
    ckpt = torch.load(ckpt_path, map_location='cuda', weights_only=False)
    hparams = ckpt['hyper_parameters']
    state_dict = ckpt['state_dict']

    # Extract the enformer backbone weights from the saved state dict
    saved_state_dict = {k: v for k, v in state_dict.items()}

    # Create a dummy artifact that provides the backbone weights from our checkpoint
    class DummyArtifact:
        def download(self, d):
            # The pretrained Enformer constructor expects the large public
            # output head. Only provide the shared backbone here; the
            # fine-tuned 3-task head is restored by load_state_dict below.
            backbone_sd = {k.removeprefix('model.'): v for k, v in saved_state_dict.items()
                           if k.startswith('model.embedding.')}
            torch.save(backbone_sd, Path(d) / 'human.h5')

    original_get_artifact = grelu.resources.get_artifact

    def patched_get_artifact(*args, **kwargs):
        return DummyArtifact()

    original_load_state_dict = torch.nn.Module.load_state_dict

    def relaxed_enformer_load_state_dict(module, load_state_dict, strict=True, assign=False):
        is_backbone_artifact = (
            module.__class__.__name__ == 'EnformerModel'
            and isinstance(load_state_dict, dict)
            and any(k.startswith('embedding.') for k in load_state_dict)
        )
        if is_backbone_artifact:
            return original_load_state_dict(module, load_state_dict, strict=False, assign=assign)
        return original_load_state_dict(module, load_state_dict, strict=strict, assign=assign)

    with patch('grelu.resources._check_wandb'):
        with patch.object(grelu.resources, 'get_artifact', patched_get_artifact):
            with patch.object(torch.nn.Module, 'load_state_dict', relaxed_enformer_load_state_dict):
...
def get_gosai_oracle(mode='train'):
    if mode == 'train':
        ...
        raise ValueError
    return _load_gosai_oracle(ckpt_path)
...
def get_atac_oracle():
    return _load_gosai_oracle(os.path.join(base_path, 'mdlm/gosai_data/binary_atac_cell_lines.ckpt'))
