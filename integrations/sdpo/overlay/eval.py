...
def eval_model_from_paths(base_path, ckpt_path, num_sample_batches, num_samples_per_batch, 
               ...
               jaspar=False, order_policy='baseline', dprm_beta=1.0, dprm_warmup_steps=100,
               dprm_switch_steps=400, dprm_ready_count=64, dprm_shortlist_size=64):
    ...
    ref_model = DiffusionSDPO.load_from_checkpoint(ref_path, config=config, beta=1.0, generator='rkl', strict=False).to('cuda')
    ref_model.eval()
    
    model_path = os.path.join(base_path, ckpt_path)
    
    model = DiffusionSDPO.load_from_checkpoint(ref_path, config=config, beta=1.0, strict=False)
...
if __name__ == '__main__':
    ...
    results = eval_model_from_paths(
        args.base_path, args.model_path, args.num_sample_batches, args.num_samples_per_batch,
        ref_path=args.ref_model_path, jaspar=args.jaspar, order_policy=args.order_policy,
        dprm_beta=args.dprm_beta, dprm_warmup_steps=args.dprm_warmup_steps,
        dprm_switch_steps=args.dprm_switch_steps, dprm_ready_count=args.dprm_ready_count,
