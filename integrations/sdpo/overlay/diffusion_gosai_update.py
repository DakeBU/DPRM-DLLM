...
class Diffusion(L.LightningModule):
  ...
  def _init_ordering(self):
    ordering = self.config.get('ordering', {}) if hasattr(self.config, 'get') else {}
    self.order_policy = str(ordering.get('policy', 'baseline'))
    self.dprm_beta = float(ordering.get('dprm_beta', 1.0))
  ...
  def _current_step_for_ordering(self):
    ...
    return int(getattr(self, '_manual_global_step', 0))

  def _dprm_gate(self, phase_idx, conf_idx):
    step = self._current_step_for_ordering()
    denom = max(self.dprm_switch_steps - self.dprm_warmup_steps, 1)
    time_gate = min(max((step - self.dprm_warmup_steps) / denom, 0.0), 1.0)
    ready = (self.dprm_count[phase_idx, conf_idx] / max(self.dprm_ready_count, 1.0)).clamp(0.0, 1.0)
    return time_gate * ready

  def _phase_index_from_mask(self, x):
    mask_frac = (x == self.mask_index).float().mean(dim=1)
    phase = ((1.0 - mask_frac) * self.dprm_phase_bins).long()
    return phase.clamp(0, self.dprm_phase_bins - 1)
  ...
  def update_dprm_stats_from_batch(self, x0, rewards, t):
    ...
    if rewards is None:
      return
    if self._current_step_for_ordering() < self.dprm_warmup_steps:
      return
    if self.T > 0:
    ...
    token_conf = torch.gather(model_output.exp(), dim=-1, index=x0[:, :, None]).squeeze(-1)
    phase = self._phase_index_from_mask(xt)
    ...
    for n in range(x0.shape[0]):
      idx = masked[n].nonzero(as_tuple=False).flatten()
      if idx.numel() == 0:
        continue
      p = int(phase[n].item())
      b = bins[n, idx]
  ...
  def _ordered_reveal_update(self, x, log_p_x0, move_chance_t, move_chance_s):
    ...
    token_probs = log_p_x0.exp()[..., :self.mask_index]
    conf, _ = token_probs.max(dim=-1)
    phase = self._phase_index_from_mask(x)
    token_samples = _sample_categorical(token_probs)
    reveal_prob = ((move_chance_t - move_chance_s) / move_chance_t.clamp_min(1e-8)).squeeze(-1).squeeze(-1)
    out = x.clone()
    for n in range(x.shape[0]):
      idx = masked[n].nonzero(as_tuple=False).flatten()
      if idx.numel() == 0:
        continue
      budget = int(torch.ceil(reveal_prob[n].clamp(0, 1) * idx.numel()).item())
      budget = max(0, min(budget, idx.numel()))
      if budget == 0:
        continue
      if self.order_policy == 'dprm_random' and self._current_step_for_ordering() < self.dprm_warmup_steps:
        perm = torch.randperm(idx.numel(), device=idx.device)[:budget]
        chosen = idx[perm]
      else:
  ...
  def _ddpm_update(self, x, t, dt, return_process=False):
    ...
    q_xs = log_p_x0.exp() * (move_chance_t
                             - move_chance_s)
    q_xs[:, :, self.mask_index] = move_chance_s[:, :, 0]
    ordered_x = self._ordered_reveal_update(x, log_p_x0, move_chance_t, move_chance_s)
    if ordered_x is not None:
      if return_process:
        copy_flag = (x != self.mask_index).to(x.dtype)
        return ordered_x, x, unet_conditioning, move_chance_t, copy_flag
      return ordered_x
    _x = _sample_categorical(q_xs)
    copy_flag = (x != self.mask_index).to(x.dtype)
    if return_process:
