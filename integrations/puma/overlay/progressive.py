...
import torch
import math
from dprm_order import BucketizedDPRMController
...
class PhasedMasking:
    ...
    def __init__(
        ...
        self.dprm_cfg = dprm_cfg or {}
        assert mode in ["standard" , "confidence_collapse"], "invalid/deprecated mode"
        assert order_policy in ["confidence", "dprm_soft_bon"], "unsupported order policy"
        ...
        if self.order_policy == "dprm_soft_bon":
            self.dprm_controller = BucketizedDPRMController(self.K, device=device, cfg=self.dprm_cfg)
    ...
    def update_k(self, new_k: int):
        ...
        if self.dprm_controller is not None:
            old_controller = self.dprm_controller
            new_controller = BucketizedDPRMController(self.K, device=self.device, cfg=self.dprm_cfg)
            overlap = min(old_controller.num_phases, new_controller.num_phases)
            new_controller.counts[:overlap] = old_controller.counts[:overlap]
            new_controller.reward_exp_sums[:overlap] = old_controller.reward_exp_sums[:overlap]
            self.dprm_controller = new_controller
    ...
    def update_with_logits(self, log_probs: torch.Tensor):
        ...
        xt_prev = self.xt
        xt = xt_prev
        log_conf = log_probs.max(dim=2)[0]
        valid_mask = mask_idx & (~self.state["prompt_mask"])
        if k_max > 0:
            if self.dprm_controller is None:
                score_conf = torch.where(valid_mask, log_conf, torch.finfo(log_probs.dtype).min)
                xt = unmask_from_scores(score_conf, to_reveal, self.x0, xt_prev)
            else:
                selected = self.dprm_controller.select_mask(
                ...
                xt = torch.where(selected, self.x0, xt_prev)
        ...
        if self.dprm_controller is not None:
            newly_revealed = (xt_prev == self.mask_id) & (xt != self.mask_id) & (~self.state["prompt_mask"])
            if newly_revealed.any():
                true_log_probs = log_probs.gather(dim=-1, index=self.x0.unsqueeze(-1)).squeeze(-1)
                denom = newly_revealed.sum(dim=1).clamp_min(1).float()
                reward_per_seq = (true_log_probs * newly_revealed).sum(dim=1) / denom
                self.dprm_controller.observe(
                    log_confidence=log_conf,
                    selected_mask=newly_revealed,
                    phases=self.state["phase"],
                    reward_per_seq=reward_per_seq.detach(),
                )
    ...
    def get_extra_state(self) -> dict:
        if self.dprm_controller is None:
            return {}
        return {"dprm_order_state": self.dprm_controller.state_dict()}
