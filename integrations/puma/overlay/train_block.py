...
def main(cfg: DictConfig):
    ...
    if strategy == "progressive":
        ...
        order_policy = getattr(train_cfg, "order_policy", "confidence")
        dprm_cfg = OmegaConf.to_container(getattr(train_cfg, "dprm", {}), resolve=True)
            ...
            if global_step % train_cfg.eval_steps == 0:
                ...
                if is_main:
                    ...
                    if global_step % train_cfg.save_steps == 0 and train_cfg.ema is not None:
                        extra = dict(val_acc_dict or {})
                        if strategy == "progressive":
                            extra.update(pool.get_extra_state())
                        saved_path = save_ema_snapshot(ckpt_dir, model, ema, cfg, epoch, global_step, val_loss = None, extra = extra)
