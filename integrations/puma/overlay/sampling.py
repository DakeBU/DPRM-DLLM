...
from dprm_order import BucketizedDPRMController, confidence_phase_from_step
...
def mdm_sampling(model, xt, mask_id, sampling_cfg, device: torch.device = None, track: bool = False, arm_init: bool = False):
    ...
    if track:
        track_xt = []
    total_steps = L // unmasking_num + 1
    dprm_controller = None
    if confidence == "dprm_soft_bon":
        dprm_controller = BucketizedDPRMController(
            num_phases=getattr(sampling_cfg, "dprm_num_phases", max(4, min(total_steps, 16))),
            device=xt.device,
            cfg={
                "num_bins": getattr(sampling_cfg, "dprm_num_bins", 16),
                "reward_beta": getattr(sampling_cfg, "dprm_reward_beta", 1.0),
                "value_scale": getattr(sampling_cfg, "dprm_lambda", 1.0),
                "warmup_steps": getattr(sampling_cfg, "dprm_warmup_steps", 0),
                "switch_steps": getattr(sampling_cfg, "dprm_switch_steps", max(total_steps // 2, 1)),
                "ready_count": getattr(sampling_cfg, "dprm_ready_count", 16),
                "sampled_shortlist": getattr(sampling_cfg, "dprm_sampled_shortlist", False),
                "candidate_multiplier": getattr(sampling_cfg, "dprm_candidate_multiplier", 4),
                "min_candidates": getattr(sampling_cfg, "dprm_min_candidates", 8),
    ...
    for i in range(L // unmasking_num + 1):
        ...
        if confidence == "top_k":
            ...
            if selected.any():
                denom = selected.sum(dim=1).clamp_min(1).float()
                pseudo_reward = (log_conf * selected).sum(dim=1) / denom
                dprm_controller.observe(log_conf, selected, phase, pseudo_reward.detach())
            if track:
...
def mdm_sampling_block(model, xt, block_size, mask_id, sampling_cfg, device: torch.device = None):
    ...
    xt = xt.clone()
    total_steps = block_size // unmasking_num + 2
    dprm_controller = None
    if confidence == "dprm_soft_bon":
        dprm_controller = BucketizedDPRMController(
            num_phases=getattr(sampling_cfg, "dprm_num_phases", max(4, min(total_steps, 16))),
