...
class ProgressiveBlock:
    ...
    def update_block(self, block_id: int, log_probs_prefix: torch.Tensor):
        ...
        # confidence / DPRM scores
        log_conf = log_probs_prefix[: , s:e , :].max(dim = 2)[0]
        valid_mask = mask_idx & (~pm_blk)
        xt_prev = xt_blk
        if self.dprm_controller is None:
            score = torch.where(valid_mask, log_conf, torch.finfo(log_conf.dtype).min)
            ...
            xt_blk = torch.where(selected, x0_blk, xt_blk)
        ...
        if self.dprm_controller is not None:
            newly_revealed = (xt_prev == self.mask_id) & (xt_blk != self.mask_id) & (~pm_blk)
            if newly_revealed.any():
                true_log_probs = log_probs_prefix[:, s:e, :].gather(dim=-1, index=x0_blk.unsqueeze(-1)).squeeze(-1)
