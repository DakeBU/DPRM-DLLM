...
def image_aux_bins(mask_index: torch.Tensor, block_mask: torch.Tensor, aux_bins: int) -> torch.Tensor:
    ...
    # T2I's 260-token block is <begin_image>, 256 visual codes, and three
...
def make_fixed_t2i_scaffold_hook(tokenizer, prompt_length: int):
    """Fix four T2I format tokens so ordering acts on 256 visual codes."""
    newline_ids = tokenizer.encode("\n", add_special_tokens=False)
    if len(newline_ids) != 1:
        raise RuntimeError(f"expected one newline token, found {newline_ids}")
    fixed = {
        0: tokenizer.convert_tokens_to_ids("<|begin_of_image|>"),
        257: tokenizer.convert_tokens_to_ids("<|end_of_image|>"),
        258: tokenizer.convert_tokens_to_ids("<|im_end|>"),
        259: int(newline_ids[0]),
    }

    def hook(_step, x: torch.Tensor, _logits):
        for offset, token_id in fixed.items():
            x[:, int(prompt_length) + int(offset)] = int(token_id)
        return x
...
def make_dprm_score_hook(
    ...
    def hook(confidence: torch.Tensor, **context) -> torch.Tensor:
        ...
        visual_mask = visual_candidate_mask(
        ...
        finite = candidate_mask & visual_mask
        ...
        score = torch.where(visual_mask, summary.score, summary.base_score)
        return score[0]
...
def make_action_value_score_hook(
    ...
    def hook(confidence: torch.Tensor, **context) -> torch.Tensor:
        ...
        candidate_canvas_indices = candidate_visual_indices(
            context["mask_index"], context["block_mask"]
        )[candidates]
        masked_visual_indices = {
            int(idx)
            for idx in candidate_canvas_indices.detach().cpu().tolist()
            if 0 <= int(idx) < 256
        }

        feature_rows = []
        for candidate, visual_index_tensor in zip(candidates, candidate_canvas_indices):
...
def make_torch_order_score_hook(
    ...
    def hook(confidence: torch.Tensor, **context) -> torch.Tensor:
        step = int(context["step"])
        if active_steps and step not in active_steps:
            return confidence
        visual = visual_candidate_mask(context["mask_index"], context["block_mask"]).to(
            confidence.device
        )
        ...
        masked_visual = torch.zeros(256, dtype=torch.bool, device=confidence.device)
        valid_visual = visual_indices[
            (visual_indices >= 0) & (visual_indices < 256)
        ]
        masked_visual[valid_visual] = True
        features = build_action_features(
            confidence=confidence,
            candidate_indices=candidates,
            visual_indices=visual_indices,
            masked_visual=masked_visual,
        ...
        adjusted, advantages = adjusted_order_scores(
            ...
            features=features,
            scorer=scorer,
            guidance_scale=float(args.dprm_action_guidance_scale),
        )
        default_choice = candidates[torch.argmax(confidence[candidates])]
        selected = candidates[torch.argmax(adjusted[candidates])]
        stats["calls"] += 1
        stats["candidate_count"] += int(candidates.numel())
        stats["predicted_advantage_sum"] += float(advantages.sum().item())
        stats["predicted_advantage_abs_sum"] += float(advantages.abs().sum().item())
        stats["selected_nonconfidence_count"] += int(selected.item() != default_choice.item())
        return adjusted
...
def make_rank_bucket_score_hook(
    ...
    def hook(confidence: torch.Tensor, **context) -> torch.Tensor:
        ...
        finite = torch.isfinite(visual_confidence)
        selected = torch.argmax(adjusted)
        default = torch.argmax(confidence)
        stats["calls"] += 1
        stats["candidate_count"] += int(finite.sum().item())
        stats["predicted_advantage_sum"] += float(reward[finite].sum().item())
        stats["predicted_advantage_abs_sum"] += float(reward[finite].abs().sum().item())
        stats["selected_nonconfidence_count"] += int(selected.item() != default.item())
        if score_snapshots is not None:
            score_snapshots[int(context["step"])] = {
                "base_score": confidence.detach().float().cpu(),
                "dprm_value": reward.detach().float().cpu(),
                "adjusted_score": adjusted.detach().float().cpu(),
                "visual_mask": visual.detach().bool().cpu(),
            }
        return adjusted
...
def make_order_observer(
    ...
    def observer(confidence: torch.Tensor, transfer_index: torch.Tensor, **context) -> None:
        ...
        trace_edges = getattr(args, "_trace_confidence_bin_edges", ())
        if trace_edges:
            edges = torch.tensor(
                trace_edges, device=conf_prob.device, dtype=conf_prob.dtype
            )
            conf_bins = torch.bucketize(conf_prob, edges, right=True)
        else:
        ...
        visual_selected = (sequence_positions - block_start >= 1) & (
            sequence_positions - block_start <= 256
        )
        ...
        snapshot = (score_snapshots or {}).pop(int(context["step"]), None)
        if snapshot is not None:
            snapshot_selected = selected.detach().cpu()
            visual_base_score = snapshot["base_score"].masked_fill(
                ~snapshot["visual_mask"], -torch.inf
            )
            default_candidate = int(torch.argmax(visual_base_score).item())
            record.update(
                {
                    "selected_base_order_scores": snapshot["base_score"][
                        snapshot_selected
                    ].tolist(),
                    "selected_dprm_values": snapshot["dprm_value"][
                        snapshot_selected
                    ].tolist(),
                    "selected_adjusted_order_scores": snapshot["adjusted_score"][
                        snapshot_selected
                    ].tolist(),
                    "confidence_default_candidate_index": default_candidate,
                    "confidence_default_base_order_score": float(
                        snapshot["base_score"][default_candidate].item()
                    ),
                    "confidence_default_dprm_value": float(
                        snapshot["dprm_value"][default_candidate].item()
                    ),
                    "confidence_default_adjusted_order_score": float(
                        snapshot["adjusted_score"][default_candidate].item()
                    ),
                }
            )
        if args.trace_provisional_phases and phase not in provisional_phases_seen:
            ...
            image_slots = block_positions[1:257]
...
def make_counterfactual_order_override(args: argparse.Namespace, stats: dict):
    ...
    if args.force_order_steps:
        if len(args.force_order_steps) != len(args.force_confidence_quantiles):
            raise ValueError("--force-order-steps and --force-confidence-quantiles must match")
        force_quantiles = dict(
            zip(
                (int(step) for step in args.force_order_steps),
                (float(value) for value in args.force_confidence_quantiles),
            )
        )
    else:
        force_quantiles = {
            int(args.force_order_step): float(args.force_confidence_quantile)
        }
    stats["requested_actions"] = len(force_quantiles)
    stats["actions"] = []
    ...
    def override(
        ...
        **context,
        ...
        step = int(context["step"])
        if step not in force_quantiles:
        ...
        quantile = float(np.clip(force_quantiles[step], 0.0, 1.0))
        if args.force_target_rank_bin is not None:
            in_target = candidates[
                rank_bins[candidates] == int(args.force_target_rank_bin)
            ]
            if in_target.numel() == 0:
                stats["failure"] = "empty_target_rank_bin"
                return transfer_index
            chosen = in_target[torch.argmax(conf_prob[in_target])]
            rank = int(torch.where(ordered == chosen)[0][0].item())
        else:
        ...
        action_stats = {
                ...
                "requested_rank_bin": args.force_target_rank_bin,
                "candidate_count": int(candidates.numel()),
        ...
        stats["applied_actions"] = len(stats["actions"])
        stats["applied"] = len(stats["actions"]) == len(force_quantiles)
        if len(stats["actions"]) == 1:
            # Preserve the single-action payload consumed by existing analyzers.
...
def main() -> None:
    ...
    parser.add_argument(
        "--dprm-order-scorer",
        default=None,
        help="Frozen train-test-matched Omni DPRM scorer artifact (.pt).",
    )
    parser.add_argument(
        "--dprm-action-steps",
        type=int,
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Decode steps for a multi-action intervention.",
    )
    parser.add_argument(
        "--force-confidence-quantiles",
        type=float,
        nargs="+",
        default=None,
        help="Ascending confidence quantiles paired with --force-order-steps.",
    )
    parser.add_argument("--force-confidence-bins", type=int, default=8)
    parser.add_argument("--force-rank-bins", type=int, default=8)
    parser.add_argument(
        "--force-target-rank-bin",
        type=int,
        default=None,
        help="Force the highest-confidence action in this within-state rank bin.",
    )
    ...
    if str(args.order_policy).startswith("dprm"):
        if args.dprm_table or args.dprm_action_value_model or args.dprm_order_scorer:
            pass
    ...
    if sum(bool(source) for source in score_sources) > 1:
        raise SystemExit("choose one DPRM score source")
    if args.dprm_table:
    ...
    elif args.dprm_order_scorer:
        if args.dprm_order_scorer.endswith(".json"):
            rank_controller, scorer_metadata = load_omni_order_controller(
            ...
            if isinstance(rank_controller, OmniBucketTableDPRM):
                ...
                args.trace_confidence_bins = rank_controller.confidence_bins
                deployed_order_controller = {
                    "type": "bucket_table",
                    "guidance_scale": rank_controller.guidance_scale,
                    "reward_temperature": rank_controller.reward_temperature,
                    "policy_warmup_steps": rank_controller.policy_warmup_steps,
                    "warmup_steps": rank_controller.warmup_steps,
                    "switch_steps": rank_controller.switch_steps,
                    "ready_count": rank_controller.ready_count,
                    "num_phases": rank_controller.num_phases,
                    "confidence_bins": rank_controller.confidence_bins,
            ...
            torch_scorer, scorer_metadata = OmniOrderScorer.load_artifact(
    ...
    generation_tokens_hook = (
        make_fixed_t2i_scaffold_hook(tokenizer, int(input_ids.shape[1]))
        if args.fixed_t2i_scaffold
        else (lambda _step, x, _logits: x)
    )
    ...
    with torch.inference_mode():
        outputs, _histories = model.generate(
            ...
            generation_tokens_hook_func=generation_tokens_hook,
    ...
    payload = {
        ...
        "fixed_t2i_scaffold": bool(args.fixed_t2i_scaffold),
    ...
    if args.dprm_action_value_model or args.dprm_order_scorer:
        ...
        if deployed_order_controller is None:
            payload["dprm_action_guidance_scale"] = args.dprm_action_guidance_scale
    if args.force_order_step is not None or args.force_order_steps:
        ...
        if args.require_forced_action and not counterfactual_stats.get("applied"):
            raise RuntimeError(
                "not all requested counterfactual actions were applied: "
    ...
    if args.trace_order_stats:
        if args.fixed_t2i_scaffold:
            selected_visual_indices = [
                ...
                for index in record.get("selected_visual_indices", [])
            ]
            if sorted(selected_visual_indices) != list(range(256)):
                raise RuntimeError(
                    "fixed T2I ordering must select each of the 256 visual "
                    "positions exactly once; observed "
                    f"{len(selected_visual_indices)} actions over "
