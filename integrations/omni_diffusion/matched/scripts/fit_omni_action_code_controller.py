#!/usr/bin/env python3
"""Fit an Omni stage/rank/code DPRM table from paired action rollouts."""
...
from dprm.omni_order import OmniStageRankCodeDPRM
...
def main() -> None:
    ...
    parser.add_argument("--rank-bins", type=int, default=64)
    parser.add_argument("--code-bins", type=int, default=4)
    ...
    if args.beta <= 0 or min(args.rank_bins, args.code_bins, args.min_count) < 1:
        raise SystemExit("temperatures and bucket settings must be positive")

    active_steps = tuple(sorted(set(args.active_steps)))
    stage_index = {step: index for index, step in enumerate(active_steps)}
    shape = (len(active_steps), args.rank_bins, args.code_bins)
    counts = [
    ...
    for row in rows:
        ...
        if row.get("rank_quantile") is not None:
            rank = min(
                args.rank_bins - 1,
                max(0, int(float(row["rank_quantile"]) * args.rank_bins)),
            )
        else:
        ...
        code = min(
            ...
            max(
                0,
                (int(row["provisional_token_id"]) - 168072)
                * args.code_bins
                // 8192,
            ),
        )
        ...
        counts[stage][rank][code] += 1
        exp_sums[stage][rank][code] += math.exp(
            args.beta * max(-20.0, min(20.0, reward))
        )
        accepted += 1
    if accepted == 0:
        raise SystemExit("no records match the requested action steps")
    ...
    for stage in range(shape[0]):
        for rank in range(shape[1]):
            for code in range(shape[2]):
                count = counts[stage][rank][code]
                if count < args.min_count:
                    continue
                mean_exp = (exp_sums[stage][rank][code] + shrinkage) / (
                    count + shrinkage
                )
                value = math.log(max(mean_exp, 1e-12)) / args.beta
                # A negative historical advantage must never displace the
                # native confidence action.
                values[stage][rank][code] = max(0.0, value)
    ...
    metadata = {
        "design": "offline action-conditioned DPRM with provisional-code auxiliary bins",
        ...
        "positive_ready_buckets": sum(
            value > 0 for stage in values for rank in stage for value in rank
        ),
        "source_summary": {
            "action_fit_prompt_count": len({str(row.get("prompt", "")) for row in rows}),
            "fixed_visual_canvas": True,
            "action_conditioned_continuations": True,
        },
        "score_contract": {
            "base_order_score": "negative_token_entropy",
            "bucket_coordinate": "within_state_confidence_rank_and_provisional_code",
