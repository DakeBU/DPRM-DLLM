...
class ModelArguments:
    ...
    language_model_trainable_last_n_layers: int = field(
        default=-1,
        metadata={
            "help": (
                "When nonnegative, freeze the language model and train only the "
                "last N decoder layers. Embeddings and the LM head remain frozen."
            )
        },
    )
...
def freeze_model_params(model_args, data_args, training_args, config, tokenizer, model):
    ...
    trainable_last_n = model_args.language_model_trainable_last_n_layers
    if trainable_last_n >= 0:
        if not hasattr(model.model, "layers"):
            raise ValueError(
                "language_model_trainable_last_n_layers requires model.model.layers"
            )
        model.requires_grad_(False)
        layers = model.model.layers
        if trainable_last_n > len(layers):
            raise ValueError(
                "language_model_trainable_last_n_layers exceeds the decoder depth: "
                f"{trainable_last_n} > {len(layers)}"
            )
        if trainable_last_n:
            for layer in layers[-trainable_last_n:]:
                layer.requires_grad_(True)
        logger.info(
            "Froze embeddings, LM head, and early decoder layers; "
            f"training the last {trainable_last_n}/{len(layers)} decoder layers."
        )
...
def main():
    ...
    if training_args.do_train:
        ...
        if os.environ.get("DPRM_SKIP_FINAL_SAVE", "0") == "1":
            logger.info("Skipping final trainer.save_model() because DPRM_SKIP_FINAL_SAVE=1")
        else:
