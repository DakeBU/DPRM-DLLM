...
def _dprm_float_env(name: str, default: float, low: float = 0.0, high: float = 1.0) -> float:
    try:
...
def forward_process(
    ...
    for i in range(b):
        ...
        if order_policy in {
            ...
            # Matched policies reconstruct clean visual targets and replace this
            # provisional random state inside the trainer after a model forward.
            t = torch.rand(valid_len, device=device)
            mask_threshold = (1 - eps) * t + eps  # [valid_len]
            rand_vals = torch.rand(valid_len, device=device)
        else:
            p_i = p_mask[i].item()
            pos = torch.linspace(0.0, 1.0, valid_len, device=device)
            jitter = torch.rand(valid_len, device=device) * 1e-4
            if order_policy in {"progressive", "progressive_confidence", "low_confidence"}:
                order_score = pos + jitter
            elif order_policy in {"dprm_confidence_warmup", "dprm", "dprm_confidence"}:
                if os.environ.get("DPRM_ALLOW_OMNI_POSITION_PROXY_DPRM", "0") != "1":
                    raise RuntimeError(
                        f"{order_policy} is only a position/jitter proxy in the current Omni data path; "
                        "do not use it for formal DPRM runs."
                    )
                pos_weight = _dprm_float_env("DPRM_CONF_POSITION_WEIGHT", 0.8)
                order_score = pos_weight * pos + (1.0 - pos_weight) * torch.rand(valid_len, device=device)
                ...
                pos_weight = _dprm_float_env("DPRM_RANDOM_POSITION_WEIGHT", 0.5)
                order_score = pos_weight * pos + (1.0 - pos_weight) * torch.rand(valid_len, device=device)
            else:
                raise RuntimeError(f"Unknown DPRM_TRAIN_ORDER_POLICY: {order_policy}")
            mask_threshold = torch.full((valid_len,), p_i, device=device)
            rand_vals = order_score
...
class Qwen2Dataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            try:
                raw_index = index % len(self.raw_data)
                sample = self.raw_data[raw_index]
                ...
                try:
                    with open(os.path.join(self.output_dir, "data_error.log"), "a") as f:
                        ...
                        #print(self.raw_data[raw_index])
                        print("-" * 100, file=f)
                        print(traceback.format_exc(), file=f)
                        print(self.raw_data[raw_index], file=f)
                except Exception as error:
                    print(error)
                return {}
...
def preprocess(
    ...
    matched_revealed = sample.get("dprm_revealed_visual_indices")
    if os.environ.get("DPRM_OMNI_PRECOMPUTED_TRAJECTORY", "0") in {"1", "hybrid"}:
        if matched_revealed is None:
            raise RuntimeError(
                "precomputed Omni training requires dprm_revealed_visual_indices"
            )
        expected_trajectory_policy = {
            "random": "random",
            "random_matched": "random",
            "confidence_matched": "progressive_confidence",
            "dprm_matched": "dprm_confidence_warmup",
        }.get(os.environ.get("DPRM_TRAIN_ORDER_POLICY", "").lower())
        observed_trajectory_policy = sample.get("dprm_trajectory_policy")
        if expected_trajectory_policy is None:
            raise RuntimeError(
                "precomputed Omni training requires random, confidence_matched, "
                "random_matched, or dprm_matched"
            )
        if observed_trajectory_policy != expected_trajectory_policy:
            raise RuntimeError(
                "precomputed Omni trajectory-policy mismatch: "
    ...
    if matched_revealed is not None:
        ...
        masked_indices[0, visual_positions] = True
        revealed = torch.as_tensor(matched_revealed, dtype=torch.long)
        revealed = revealed[(revealed >= 0) & (revealed < visual_positions.numel())]
        if revealed.numel():
            masked_indices[0, visual_positions[revealed]] = False
        p_mask = masked_indices.float().mean(dim=1, keepdim=True)
    else:
