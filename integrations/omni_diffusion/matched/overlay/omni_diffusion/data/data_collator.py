...
def collate_fn_deepspeed(batch):
    ...
    if "images" in batch[0].keys():
        has_nonempty_images = all(
            isinstance(x.get("images"), torch.Tensor) and x["images"].numel() > 0
            for x in batch
        )
        if has_nonempty_images:
    ...
    if "attention_mask" in new_batch:
        max_valid_length = int(new_batch["attention_mask"].sum(dim=1).max().item())
        for key in ("tokens", "labels", "input_ids", "position_ids", "attention_mask"):
            if key in new_batch and torch.is_tensor(new_batch[key]) and new_batch[key].dim() == 2:
                new_batch[key] = new_batch[key][:, :max_valid_length]
        if "max_seq_len" in new_batch and torch.is_tensor(new_batch["max_seq_len"]):
            new_batch["max_seq_len"] = torch.clamp(new_batch["max_seq_len"], max=max_valid_length)
