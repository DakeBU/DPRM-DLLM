...
: "${OMNI_DUAL_TABLE_EQUAL:?set the equal-weight dual-CLIP table}"
...
GPUS_TEXT="${OMNI_DEVELOPMENT_GPUS:-0 1 2 3 4 7}"
...
# The first development sweep localized the useful intervention to the middle
# denoising stages. This sweep changes only terminal-utility weights and two
# conservative score-mixing settings; it never accesses the confirmation split.
specs=(
  "eq_g075_m020|${OMNI_DUAL_TABLE_EQUAL}|0.075|0.020"
  "eq_g075_m030|${OMNI_DUAL_TABLE_EQUAL}|0.075|0.030"
  "eq_g100_m020|${OMNI_DUAL_TABLE_EQUAL}|0.100|0.020"
  "eq_g100_m030|${OMNI_DUAL_TABLE_EQUAL}|0.100|0.030"
  "bh_g075_m020|${OMNI_DUAL_TABLE_B_HEAVY}|0.075|0.020"
  "bh_g075_m030|${OMNI_DUAL_TABLE_B_HEAVY}|0.075|0.030"
  "bh_g100_m020|${OMNI_DUAL_TABLE_B_HEAVY}|0.100|0.020"
  "bh_g100_m030|${OMNI_DUAL_TABLE_B_HEAVY}|0.100|0.030"
)
labels=(confidence)
controllers=("")
...
for spec in "${specs[@]}"; do
  IFS='|' read -r label table guidance gap <<< "${spec}"
  ...
  "${PYTHON}" "${SCRIPT_DIR}/freeze_omni_bucket_controller.py" \
    ...
    --reward-action-steps 96 112 128 144 160 --max-base-score-gap "${gap}"
  labels+=("${label}")
...
{
  "design": "dual-encoder middle-stage controller development",
  "claim_eligible": false,
  "prompt_offset": ${OFFSET},
  "prompt_count": ${COUNT},
  "prompt_file": ${PROMPT_FILE_JSON},
  "prompt_file_sha256": ${PROMPT_FILE_SHA_JSON},
  "equal_table_sha256": "$(sha256sum "${OMNI_DUAL_TABLE_EQUAL}" | awk '{print $1}')",
  "b_heavy_table_sha256": "$(sha256sum "${OMNI_DUAL_TABLE_B_HEAVY}" | awk '{print $1}')",
  "shared_checkpoint": "${OMNI_MODEL_PATH}",
  "selection_metrics": ["CLIP-L/14 paired lower CI", "CLIP-B/32 non-regression"],
...
"${PYTHON}" "${SCRIPT_DIR}/select_omni_dual_clip_controller.py" \
  ...
  --min-prompt-override-fraction 0.10 --require-positive-primary-ci
"${PYTHON}" "${SCRIPT_DIR}/prepare_omni_formal_controller.py" \
  ...
  --source-table-root "$(dirname "${OMNI_DUAL_TABLE_EQUAL}")" \
  --table-prompt-range 2000 2047 "${SELECTION_PROMPT_ARGS[@]}" \
