...
def select_transfer_indices(
    ...
    step_idx=0,
    warmup_steps=0,
    dprm_score_hook_func=None,
    hook_context=None,
):
    """Select which currently masked positions to reveal under a named order policy."""
    order_policy = str(order_policy or "confidence").lower()
    valid = torch.isfinite(confidence)
    valid_indices = torch.where(valid)[0]
    if valid_indices.numel() == 0:
        return torch.empty(0, dtype=torch.long, device=confidence.device)

    k = min(int(number_transfer_tokens), int(valid_indices.numel()))
    if k <= 0:
        return torch.empty(0, dtype=torch.long, device=confidence.device)

    def choose_random():
        perm = torch.randperm(valid_indices.numel(), device=confidence.device)
        return valid_indices[perm[:k]]
    ...
    def choose_by_scores(scores):
        ...
        if alg_temp is None or alg_temp == 0:
            ...
            return selected
        probs = F.softmax(scores / alg_temp, dim=-1)
        return torch.multinomial(probs, num_samples=k)

    if order_policy in {"random", "baseline_random", "random_ordered"}:
        return choose_random()

    if order_policy in {"confidence", "progressive", "progressive_confidence", "low_confidence"}:
        return choose_by_scores(confidence)

    if order_policy in {"dprm_confidence_warmup", "dprm", "dprm_confidence"}:
    ...
    if dprm_score_hook_func is not None:
        scores = dprm_score_hook_func(confidence=confidence, **(hook_context or {}))
        return choose_by_scores(scores)

    if os.environ.get("DPRM_ALLOW_DPRM_CONFIDENCE_FALLBACK", "0") == "1":
        return choose_by_scores(confidence)

    # A DPRM-labeled policy must consume a DPRM score hook after warm-up. Silent
    # confidence fallback makes the resulting run indistinguishable from the
    # confidence baseline and invalidates the order label.
    raise RuntimeError(
        f"{order_policy} requires generation_order_score_hook_func after warm-up; "
        "set DPRM_ALLOW_DPRM_CONFIDENCE_FALLBACK=1 only for diagnostic fallback runs."
    )
...
class DreamGenerationMixin:
    ...
    def _sample(
        ...
        **kwargs,
        ...
        order_policy = kwargs.pop("order_policy", kwargs.pop("reveal_order", "confidence"))
        dprm_warmup_steps_arg = kwargs.pop("dprm_warmup_steps", None)
        generation_order_score_hook_func = kwargs.pop("generation_order_score_hook_func", None)
        generation_order_observer_func = kwargs.pop("generation_order_observer_func", None)
        generation_order_override_func = kwargs.pop("generation_order_override_func", None)
        ...
        if dprm_warmup_steps_arg is None:
            dprm_warmup_steps = max(1, int(total_steps * 0.25))
        else:
            dprm_warmup_steps = int(dprm_warmup_steps_arg)
        global_step = 0
                ...
                if alg == 'origin':
                    ...
                    if number_transfer_tokens > 0:
                        ...
                        transfer_index = select_transfer_indices(
                            ...
                            dprm_score_hook_func=generation_order_score_hook_func,
                            hook_context=order_context,
                        )
                        if generation_order_override_func is not None:
                            overridden = generation_order_override_func(
                            ...
                            if overridden is not None:
                                overridden = overridden.to(confidence.device).long().view(-1)
                                overridden = overridden[
                                    (overridden >= 0)
                                    & (overridden < confidence.numel())
                                ]
                                overridden = overridden[torch.isfinite(confidence[overridden])]
                                if overridden.numel() != min(
                                    int(number_transfer_tokens), int(torch.isfinite(confidence).sum().item())
                                ):
                                    raise RuntimeError(
                                        "generation_order_override_func returned the wrong number of valid candidates"
                                    )
                                transfer_index = overridden
                        if generation_order_observer_func is not None:
                            generation_order_observer_func(
                                confidence=confidence,
                                transfer_index=transfer_index,
                                number_transfer_tokens=number_transfer_tokens,
                                order_policy=order_policy,
