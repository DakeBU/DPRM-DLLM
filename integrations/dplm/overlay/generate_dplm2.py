...
def load_generation_model(model_name, bit_model=False):
    model_cls = DPLM2Bit if bit_model else DPLM2
    is_local_checkpoint = os.path.isfile(model_name) and model_name.endswith(
        ".ckpt"
    )
    if is_local_checkpoint:
        return model_cls.from_pretrained(
            model_name, from_huggingface=False
        )
...
def unconditional_generate(args):
    model = load_generation_model(
        args.model_name, bit_model=args.bit_model
    )
