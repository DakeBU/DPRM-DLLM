...
class DMPOConfig(GRPOConfig):
    ...
    sampler_remasking: str = field(
        ...
        metadata={"help": "Remasking strategy for non-roar samplers, "
                          "including llada, pd, pd_cache_prefix, pd_cache_dual. "
                          "Can be 'low_confidence', 'random', or 'dprm_soft_bon'."},
    )
    ...
    loss_mask_sampler: str = field(
        default="random",
        metadata={"help": "Mask-state sampler for the denoising surrogate. Can be 'random' or 'progressive'."}
    )
    loss_progressive_k: int = field(
        default=8,
        metadata={"help": "Number of progressive unmasking stages when loss_mask_sampler='progressive'."}
    )
    loss_progressive_threshold: Optional[float] = field(
        default=None,
        metadata={"help": "Optional confidence threshold for PUMA-style confidence collapse. Disabled when null."}
    )
    loss_progressive_phase_init: str = field(
        default="random",
        metadata={"help": "How to initialize progressive phases. Can be 'random' or 'zero'."}
    )
    loss_progressive_order_policy: str = field(
        default="confidence",
        metadata={"help": "Order policy for progressive unmasking. Can be 'confidence' or 'dprm_soft_bon'."}
    )
    loss_progressive_dprm_bins: int = field(
        default=16,
        metadata={"help": "Number of confidence bins for the online DPRM estimator."}
    )
    loss_progressive_dprm_reward_temperature: float = field(
        default=1.0,
        metadata={"help": "Temperature beta in log E[exp(beta * reward) | state] for the DPRM estimator."}
    )
    loss_progressive_dprm_lambda: float = field(
        default=1.0,
        metadata={"help": "Lambda used when reweighting reveal scores by the estimated DPRM value."}
    )
    loss_progressive_dprm_warmup_steps: int = field(
        default=500,
        metadata={"help": "Before this many global steps, DPRM reveal order falls back to confidence-based unmasking."}
    )
    loss_progressive_dprm_switch_steps: int = field(
        default=2000,
    ...
    loss_progressive_dprm_mode: str = field(
        default="sampled",
        metadata={"help": "DPRM reveal implementation. 'analytic' uses adjusted top-k; 'sampled' uses a shortlist soft-BoN."}
    )
    loss_progressive_dprm_candidate_multiplier: int = field(
        default=4,
        metadata={"help": "Shortlist size multiplier for sampled DPRM soft-BoN."}
    )
    loss_progressive_dprm_max_candidates: int = field(
        default=32,
        metadata={"help": "Maximum shortlist size for sampled DPRM soft-BoN."}
    )
    loss_progressive_dprm_min_candidates: int = field(
        default=8,
        metadata={"help": "Minimum shortlist size for sampled DPRM soft-BoN."}
    )
    ...
    ada_coeff: bool = field(
        default=False,
        metadata={"help": r"If True, use adaptive coeff (related with the step size \eta) based on ESS instead of the default one above"}
    )
