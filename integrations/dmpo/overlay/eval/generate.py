            ...
            for i in range(steps_per_block):
                ...
                if remasking == "dprm_soft_bon":
                    ...
                    transfer_index, _ = dprm_estimator.select_positions(
                        ...
                        num_select=num_transfer_tokens[:, i],
                        phase=phase,
...
def generate_with_prefix_cache(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
             remasking='low_confidence', mask_id=126336, threshold=None, factor=None, dprm_estimator=None,
             dprm_global_step=None, dprm_force_full=False):
    '''
    Args:
    ...
    for num_block in range(num_blocks):
        ...
        if factor is None:
            x0, transfer_index = get_transfer_index(
                ...
                dprm_global_step=dprm_global_step,
...
def get_transfer_index_dynamic(logits, temperature, remasking, mask_index, x, num_transfer_tokens, factor=1):
    ...
    x0 = torch.argmax(logits_with_noise, dim=-1) # b, l
    if remasking == "dprm_soft_bon":
        raise NotImplementedError("dprm_soft_bon is not supported with dynamic factor remasking")
    x0_p = _resolve_remasking_scores(logits, x0, remasking)
