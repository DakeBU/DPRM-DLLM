...
LEVEL_SPECS = [
    ("overall", "Math pass@K: {vanilla_label} vs {progressive_label}", None),
    ("trivial_1", "Trivial (1) Math pass@K: {vanilla_label} vs {progressive_label}", 1),
    ("easy_2", "Easy (2) Math pass@K: {vanilla_label} vs {progressive_label}", 2),
    ("medium_3", "Medium (3) Math pass@K: {vanilla_label} vs {progressive_label}", 3),
    ("hard_4", "Hard (4) Math pass@K: {vanilla_label} vs {progressive_label}", 4),
    ("ood_5", "OOD (5) Math pass@K: {vanilla_label} vs {progressive_label}", 5),
]
...
def verify_pair_compatible(meta_a, levels_a, meta_b, levels_b):
    for key in ["ks", "selected_indices", "num_examples"]:
...
def main():
    ...
    for split_name, title_template, level in LEVEL_SPECS:
        ...
        meta = {
            ...
            "extra_checkpoint": None if extra_meta is None else extra_meta.get("checkpoint"),
            ...
            "levels": [1, 2, 3, 4, 5] if level is None else [int(level)],
        }

        prefix = f"passk_math_{split_name}_random_vs_progressive"
        ...
        if base_meta is not None:
            ...
            title_parts = [args.base_label, args.vanilla_label, args.progressive_label]
            if extra_meta is not None:
                title_parts.append(args.extra_label)
            title = f"{title}: " + " vs ".join(title_parts)
        else:
