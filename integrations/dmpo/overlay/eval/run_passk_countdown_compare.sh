...
RANDOM_RUN_DIR="${RANDOM_RUN_DIR:-/var/tmp/nitanda_sub_dmpo/outputs/countdown/random/dmpo-countdown-random-5000-safe}"
PROGRESSIVE_RUN_DIR="${PROGRESSIVE_RUN_DIR:-/var/tmp/nitanda_sub_dmpo/outputs/countdown/progressive/dmpo-countdown-progressive-5000-safe}"
EXTRA_RUN_DIR="${EXTRA_RUN_DIR:-}"
OUTPUT_DIR="${OUTPUT_DIR:-/var/tmp/nitanda_sub_dmpo/eval_outputs/passk_countdown_step5000}"
DATASET_JSONL="${DATASET_JSONL:-}"
GPU_IDS="${GPU_IDS:-3,4}"
VANILLA_GPU_ID="${VANILLA_GPU_ID:-}"
PROGRESSIVE_GPU_ID="${PROGRESSIVE_GPU_ID:-}"
BASE_GPU_ID="${BASE_GPU_ID:-}"
EXTRA_GPU_ID="${EXTRA_GPU_ID:-}"
KS="${KS:-1 2 4 8 16 32}"
TEST_SIZE="${TEST_SIZE:-5120}"
...
EXTRA_LABEL="${EXTRA_LABEL:-DMPO-DPRM}"
EXTRA_REMASKING="${EXTRA_REMASKING:-dprm_soft_bon}"
EXTRA_DPRM_ESTIMATOR_PATH="${EXTRA_DPRM_ESTIMATOR_PATH:-}"
...
if [[ -z "$VANILLA_GPU_ID" || -z "$PROGRESSIVE_GPU_ID" ]]; then
  echo "Need two GPU ids. Set GPU_IDS=3,4 or set VANILLA_GPU_ID / PROGRESSIVE_GPU_ID." >&2
  exit 1
fi
...
EXTRA_STATE_DIR="${EXTRA_STATE_DIR:-$OUTPUT_DIR/raw/extra_step${COMMON_STEP:-latest}}"
...
if [[ "$INCLUDE_BASE" == "true" ]]; then
  if [[ -n "$BASE_GPU_ID" && "$BASE_GPU_ID" != "$VANILLA_GPU_ID" && "$BASE_GPU_ID" != "$PROGRESSIVE_GPU_ID" ]]; then
    ...
    echo "  ${BASE_LABEL}: sequential on GPU ${BASE_GPU_ID:-$VANILLA_GPU_ID}"
  fi
fi
if [[ "$INCLUDE_EXTRA" == "true" ]]; then
  echo "  ${EXTRA_LABEL}: sequential on GPU ${EXTRA_GPU_ID:-$VANILLA_GPU_ID}"
fi
...
run_eval_job() {
  ...
  CUDA_VISIBLE_DEVICES="$gpu_id" python "$PROJECT_ROOT/eval_passk_countdown_single.py" \
...
if [[ "$VANILLA_GPU_ID" == "$PROGRESSIVE_GPU_ID" ]]; then
  run_eval_job \
    "$VANILLA_GPU_ID" \
    "$RANDOM_CKPT" \
    "$VANILLA_LABEL" \
    "$VANILLA_STATE_DIR" \
    "$OUTPUT_DIR/vanilla_eval.log" \
    "$VANILLA_REMASKING" \
    "$VANILLA_DPRM_ESTIMATOR_PATH" || VANILLA_STATUS=$?
  ...
  if [[ "$VANILLA_STATUS" -eq 0 ]]; then
    run_eval_job \
      ...
      "$PROGRESSIVE_CKPT" \
      "$PROGRESSIVE_LABEL" \
      "$PROGRESSIVE_STATE_DIR" \
      "$OUTPUT_DIR/progressive_eval.log" \
      "$PROGRESSIVE_REMASKING" \
      "$PROGRESSIVE_DPRM_ESTIMATOR_PATH" || PROGRESSIVE_STATUS=$?
  fi
else
  (
    run_eval_job \
  ...
  (
    run_eval_job \
      ...
      "$OUTPUT_DIR/progressive_eval.log" \
      "$PROGRESSIVE_REMASKING" \
      "$PROGRESSIVE_DPRM_ESTIMATOR_PATH"
  ) &
  PROGRESSIVE_PID=$!
...
if [[ "$INCLUDE_BASE" == "true" && "$VANILLA_STATUS" -eq 0 && "$PROGRESSIVE_STATUS" -eq 0 ]]; then
  BASE_TARGET_GPU="${BASE_GPU_ID:-$VANILLA_GPU_ID}"
  run_eval_job \
    "$BASE_TARGET_GPU" \
    "" \
    "$BASE_LABEL" \
    "$BASE_STATE_DIR" \
    "$OUTPUT_DIR/base_eval.log" \
    "$BASE_REMASKING" \
    "$BASE_DPRM_ESTIMATOR_PATH" || BASE_STATUS=$?
fi

if [[ "$INCLUDE_EXTRA" == "true" && "$VANILLA_STATUS" -eq 0 && "$PROGRESSIVE_STATUS" -eq 0 && "$BASE_STATUS" -eq 0 ]]; then
  if [[ -z "$EXTRA_RUN_DIR" ]]; then
    echo "INCLUDE_EXTRA=true but EXTRA_RUN_DIR is empty." >&2
    exit 1
  fi
  if [[ -n "$COMMON_STEP" ]]; then
    EXTRA_CKPT="$(checkpoint_at_step "$EXTRA_RUN_DIR" "$COMMON_STEP")"
  else
    EXTRA_CKPT="$(latest_checkpoint "$EXTRA_RUN_DIR")"
  fi
  EXTRA_TARGET_GPU="${EXTRA_GPU_ID:-${BASE_GPU_ID:-$VANILLA_GPU_ID}}"
  run_eval_job \
    "$EXTRA_TARGET_GPU" \
    "$EXTRA_CKPT" \
    "$EXTRA_LABEL" \
    "$EXTRA_STATE_DIR" \
    "$OUTPUT_DIR/extra_eval.log" \
    "$EXTRA_REMASKING" \
    "$EXTRA_DPRM_ESTIMATOR_PATH" || EXTRA_STATUS=$?
fi

if [[ "$VANILLA_STATUS" -ne 0 || "$PROGRESSIVE_STATUS" -ne 0 || "$BASE_STATUS" -ne 0 || "$EXTRA_STATUS" -ne 0 ]]; then
  ...
  if [[ "$INCLUDE_EXTRA" == "true" ]]; then
    echo "  ${EXTRA_LABEL} status: $EXTRA_STATUS" >&2
  fi
  exit 1
fi
...
if [[ "$INCLUDE_EXTRA" == "true" ]]; then
  COMBINE_ARGS+=(
    --extra_state_dir "$EXTRA_STATE_DIR"
    --extra_label "$EXTRA_LABEL"
  )
fi

python "$PROJECT_ROOT/combine_passk_countdown.py" "${COMBINE_ARGS[@]}"
